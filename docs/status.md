...
| UI | shadcn. Пауза по клику. Drawer: порядок, типографика, теги, реакции, даты, демки, ключевые слова |
| Фильтры | GraphQL `lyrics(..., demosOnly, referencesOnly)`; демки = `#текст_с_демки`; эталон = `Lyrics.isReference` (приложение, не TG) |
