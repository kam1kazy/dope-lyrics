...
## D. Инструмент — сейчас (остаток)

Первый срез на клиенте **сделан**: порядок, теги (OR), ключевые слова, слайдеры текста и скорости. [`tasks/003_catalog_controls.md`](./tasks/003_catalog_controls.md).

Дальше: аргументы в GraphQL, эмодзи, даты, полка эталонов. Не грузить всё разом, если можно отфильтровать на сервере.
