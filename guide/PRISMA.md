...
5. Выполняет посев (заливает данные) - сейчас в этом нет необходимости, так как нет данных полученных ботом (они будут в папке bot-data), но если потребуется, то выполняем команду, чтобы добавить пользователей в базу:
```bash
bun seed
```
