...
`CarouselHistory`: `lyricIds Int[]`, `previewText`, `source` (`SHUFFLE` | `QUEUE` | `GENERATOR` | `AI`), `isLiked`, `createdAt`. Не FK на Lyrics. Лимит 12 в сервисе при insert. Клиент в этом change пишет только `QUEUE`: снимок — id очереди перед «Очистить». Шаффл каталога и «Перемешать заново» историю не трогают. Воспроизведение: `lyricsByIds` с сохранением порядка → mode queue. Enum `SHUFFLE` в схеме остаётся для старых записей.

Альтернатива писать шаффл каталога целиком — превращает весь корпус в очередь и помечает все строки «в карусели».
...
- [Старые снимки `SHUFFLE`] → уже записанные остаются в списке; новые не появляются.
